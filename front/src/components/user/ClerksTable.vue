...
<template>
  <div class="w-1/2 p-7 flex flex-col justify-center items-center bg-white shadow-lg">
    ...
    <div v-if="!loading" class="flex flex-col justify-center items-center gap-5 w-full">
      ...
      <Pagination v-slot="{ page }" :total="Math.ceil(pagination.total / pagination.perPage)"
        :default-page="pagination.page" :sibling-count="1" show-edges>
        <PaginationList v-slot="{ items }" class="flex items-center gap-1">
          ...
          <PaginationNext @click="onPageChange(pagination.page + 1)"
            :disabled="pagination.page === Math.ceil(pagination.total / pagination.perPage)" />
          <PaginationLast @click="onPageChange(Math.ceil(pagination.total / pagination.perPage))"
            :disabled="pagination.page === Math.ceil(pagination.total / pagination.perPage)" />
        </PaginationList>
      </Pagination>
    </div>
